    ...
    <LayoutView>
      <div class="header-titulo">
        ...
        <div class="header-actions">
          <router-link :to="{ name: 'report/create' }" class="btn-crear">+ Crear Reporte</router-link>
        </div>
      </div>
...
import { useReportList, useGenerateReport, useGenerateMultipleReports, useChangeStatusReport } from '../composables/useReports.js';
...
const { data: reportsData } = useReportList(payload);
...
const { mutate: generateMultiple } = useGenerateMultipleReports();
    ...
    generateMultiple(
        selectedReports.value,
        {
            onSuccess: (response) => {
                ...
                link.setAttribute("download", `reportes_multiples_${fecha}.zip`);
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
                selectedReports.value = [];
...
const get_clients = () => {}; // Los clientes ya se cargan reactivamente con useParamClients
